...
class qq_login{
	...
	public function wxlogin(){
		$uuid=empty($_GET['uuid'])?exit('{"saveOK":-1,"msg":"uuid不能为空"}'):$_GET['uuid'];
		$last=isset($_GET['last'])?'&last='.$_GET['last']:null;
				...
				if($data) {
					...
					if($mkey = $mkey[1]){
						...
						if($match[1]){
							...
							exit('ptuiCB("6","'.$uin.'","登录成功，回调百度失败！");');
						}
					}else{
						exit('ptuiCB("6","'.$uin.'","登录成功，获取mkey失败！");');
					}
				}
				if($BDUSS[1] && $STOKEN[1] && $PTOKEN[1]){
